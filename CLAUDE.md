...
All six blockers must be checked consistently across all three scripts:
...
- `$Config_RequireUpdateRing` (`$false`) — require Intune Update Ring policy delivery via PolicyManager Providers path
- `$Config_RequireMDMEnrollment` (`$false`) — require active MDM enrollment (`EnrollmentState=1`)
- `$Config_MaxScanAgeDays` (`0` = disabled) — flag non-compliant if WU client hasn't scanned in N days
...
All flags are at the top of WUDUP-Detect.ps1 and default to off (backward-compatible):
- `$Config_RequireUpdateRing = $false` — require Intune/co-mgmt Update Ring policy delivery
- `$Config_RequireMDMEnrollment = $false` — require active MDM enrollment
- `$Config_MaxScanAgeDays = 0` — maximum days since last WU scan (0 = disabled)
